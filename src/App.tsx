...
const usd_format = (val,show_digits=null) => {
...
class App extends React.Component<IAppProps, IAppState> {
  ...
  async handleChangeInput(key,value):void {
    this.setState({ loan: await Loan.set(key,value) })
  }
  async handleClickStep(key,step,increment):void {
    this.setState({ loan: await Loan.crunchTheNumbers(key,step,increment) });
  }
  render(): React.ReactNode { return (
  <div className="App py-10 px-3">

  <div className="max-w-3xl	leading-4 mx-auto mb-5">
    <h1 className="text-slate-500 font-bold">Home Loan Calculator</h1>
  </div>
  ...
  <div className="flex space-x-6 bg-white rounded-t-lg py-6 px-8">
    <div className="basis-1/3">
      ...
      <div>
        <div className="text-slate-600 font-bold text-sm">{parseInt(this.state.loan.calculations.loan_months_to_save_down_payment)} Months</div>
  ...
  <div className="flex space-x-6 bg-slate-100 border-slate-200 border-t-2 rounded-b-lg py-6 px-8">
    <div className="basis-1/3">
      <FormField id="income_yearly_in" label="Salary" description="Yearly" min={0} step={5000} type="number" value={this.state.loan.inputs.income_yearly_in} do_step={(id,step,increment) => { this.handleClickStep(id,step,increment) }} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="income_monthly_pretax_out" label="Pre-tax" description="Monthly" min={0} step={10} type="number" value={this.state.loan.inputs.income_monthly_pretax_out} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="income_tax_rate" label="Tax Rate" min={0} step={0.1} type="float" value={this.state.loan.inputs.income_tax_rate} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="income_monthly_expenses" label="Expenses" description="Monthly" type="number" min={0} step={100} value={this.state.loan.inputs.income_monthly_expenses} change={(id,value) => { this.handleChangeInput(id,value) }} />
    </div>
    <div className="basis-1/3">
      <FormField id="home_price" label="Home Price" type="number" min={0} step={5000} value={this.state.loan.inputs.home_price} do_step={(id,step,increment) => { this.handleClickStep(id,step,increment) }} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="loan_down_payment" label="Amount Down" type="number" min={0} step={5000} value={this.state.loan.inputs.loan_down_payment} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="loan_rate_interest" label="Interest Rate" type="float" min={0} step={0.1} value={this.state.loan.inputs.loan_rate_interest} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="loan_term_years" label="Term" description="Years" type="float" step={15} min={15} max={30} value={this.state.loan.inputs.loan_term_years} change={(id,value) => { this.handleChangeInput(id,value) }} />
    </div>
    <div className="basis-1/3">
      <FormField id="home_taxes_yearly" label="Taxes" description="Yearly" type="number" min={0} step={1000} value={this.state.loan.inputs.home_taxes_yearly} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="home_insurance_monthly" label="Insurance &bull; HOA" description="Monthy" type="number" min={0} step={100} value={this.state.loan.inputs.home_insurance_monthly} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="loan_rate_pmi" label="PMI Rate" type="float" min={0} step={0.1} value={this.state.loan.inputs.loan_rate_pmi} change={(id,value) => { this.handleChangeInput(id,value) }} />
      <FormField id="home_rent_monthly" label="Rental Rate" description="Monthly" type="number" min={0} step={1200} value={this.state.loan.inputs.home_rent_monthly} change={(id,value) => { this.handleChangeInput(id,value) }} />
    </div>
  </div>
